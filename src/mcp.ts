...
server.registerTool("write_diagram", { description: "Create or replace an Excalidraw JSON file. Omit projectId for the legacy DRAW_LOCAL_ROOT.", inputSchema: { path: z.string().min(1), projectId: z.string().min(1).optional(), document: z.record(z.string(), z.unknown()) } }, async ({ path, projectId, document }) => { if (projectId) await workspace.writeProjectFile(projectId, path, document); else await workspace.write(path, document); return text(`Saved ${path}`); });
server.registerTool("rename_diagram", { description: "Rename/move a drawing. Omit projectId for the legacy DRAW_LOCAL_ROOT.", inputSchema: { from: z.string().min(1), to: z.string().min(1), projectId: z.string().min(1).optional() } }, async ({ from, to, projectId }) => { if (projectId) await workspace.renameProjectFile(projectId, from, to); else await workspace.rename(from, to); return text(`Renamed ${from} -> ${to}`); });
server.registerTool("delete_diagram", { description: "Delete a drawing. Omit projectId for the legacy DRAW_LOCAL_ROOT.", inputSchema: { path: z.string().min(1), projectId: z.string().min(1).optional() } }, async ({ path, projectId }) => { if (projectId) await workspace.removeProjectFile(projectId, path); else await workspace.remove(path); return text(`Deleted ${path}`); });
server.registerTool("git_status", { description: "Read Git status; projectId returns project-scoped structured status.", inputSchema: { projectId: z.string().min(1).optional() } }, async ({ projectId }) => text(projectId ? await workspace.gitContext(projectId) : await workspace.gitStatus()));
