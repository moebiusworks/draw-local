...
export class Workspace {
  ...
  private async withLock<T>(identity: string, action: () => Promise<T>): Promise<T> {
    ...
    try { return await action(); } finally { release(); if (this.locks.get(identity) === queued) this.locks.delete(identity); }
  }
  ...
  async registerProject(directory: string, name?: string): Promise<Project> {
    ...
    return this.withLock(`registry:${this.configPath}`, async () => {
      ...
      const project = { id: randomUUID(), path: canonical, name: name?.trim() || path.basename(canonical) || canonical };
  ...
  private async writeRoot(root: string, relative: string, value: unknown, expectedRevision?: string, exclusive = false) {
    this.assertFile(relative); this.valid(value, relative);
    ...
    return this.withLock(`write:${target}`, async () => {
      ...
      try { source = await fs.readFile(target, "utf8"); } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
      if (exclusive && source !== undefined) throw new Error("Destination already exists.");
      if (expectedRevision !== undefined && (source === undefined || this.revision(source) !== expectedRevision)) throw new Error("File changed outside draw-local. Your local copy is still recoverable.");
      ...
      if (expectedRevision !== undefined) {
        ...
        try { latest = await fs.readFile(target, "utf8"); } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
        if (latest === undefined || this.revision(latest) !== expectedRevision) throw new Error("File changed outside draw-local. Your local copy is still recoverable.");
      }
  ...
  async saveDraft(id: string, projectId: string, relative: string, value: unknown) {
    ...
    try {
      ...
      if ((error as Error).message !== "Destination already exists.") throw error;
      ...
      if (JSON.stringify(target.document) !== JSON.stringify(draft.document)) throw error;
      await fs.unlink(this.resolve(this.draftsPath, `${id}.excalidraw`));
  ...
  async removeProjectFile(id: string, relative: string, expectedRevision?: string) {
    const root = await this.projectRoot(id); this.assertFile(relative); const target = this.resolve(root, relative);
    return this.withLock(`write:${target}`, async () => {
      await this.assertNoSymlink(root, target);
      const source = await fs.readFile(target, "utf8");
      if (expectedRevision !== undefined && this.revision(source) !== expectedRevision) throw new Error("File changed outside draw-local. Your local copy is still recoverable.");
      await fs.unlink(target);
    });
  }
  async renameProjectFile(id: string, from: string, to: string, expectedRevision?: string) {
    const root = await this.projectRoot(id); this.assertFile(from); this.assertFile(to);
    const source = this.resolve(root, from), target = this.resolve(root, to);
    return this.withLock(`rename:${source}`, async () => {
      await this.assertNoSymlink(root, source); await this.assertNoSymlink(root, target);
      const contents = await fs.readFile(source, "utf8");
      if (expectedRevision !== undefined && this.revision(contents) !== expectedRevision) throw new Error("File changed outside draw-local. Your local copy is still recoverable.");
      try { await fs.lstat(target); throw new Error("Destination already exists."); } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
      await fs.mkdir(path.dirname(target), { recursive: true, mode: 0o700 }); await fs.rename(source, target);
  ...
  async gitContext(id: string) {
    ...
    try {
      ...
      for (let i = 0; i < records.length; i++) {
        const item = records[i]; if (!item) continue;
        const index = item[0], worktree = item[1]; let filename = item.slice(3);
        if (index === "R" || index === "C") i++; // porcelain -z follows a rename/copy with its source path
        if (prefix) { if (!filename.startsWith(`${prefix}/`)) continue; filename = filename.slice(prefix.length + 1); }
        const conflicted = index === "U" || worktree === "U" || ["DD", "AU", "UD", "UA", "DU", "AA", "UU"].includes(index + worktree);
        const label = index === "?" ? "Untracked" : index === "!" ? "Ignored" : conflicted ? "Conflicted" : `${index !== " " ? "Staged" : ""}${index !== " " && worktree !== " " ? "; " : ""}${worktree !== " " ? "Modified" : ""}` || "Committed";
        statuses[filename] = { index, worktree, label };
      }
      return { available: true, branch, statuses, repo };
    } catch { return { available: false, branch: undefined, statuses: {} }; }
  }
