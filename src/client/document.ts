...
export function mergeDocument(
  ...
  const previousAppState = previous.appState && typeof previous.appState === "object" && !Array.isArray(previous.appState)
  ...
  const mergedElements = elements.map((element) => {
    if (!element || typeof element !== "object" || typeof (element as { id?: unknown }).id !== "string") return element;
